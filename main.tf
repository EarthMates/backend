...
resource "aws_instance" "web" {
  ami                    = "ami-0d3a2960fcac852bc" # Amazon Linux
  ...
  user_data = <<-EOF
    ...
    # Authenticate to ECR
    docker login -u AWS -p "$(aws ecr get-login-password --region eu-north-1)" 767397811928.dkr.ecr.eu-north-1.amazonaws.com

    # Pull the Docker image from ECR
    docker pull 767397811928.dkr.ecr.eu-north-1.amazonaws.com/deployment-backend:latest
    

    # Run the Docker image
    docker run -d -p 80:8080 \
    --env SECRET_KEY=${var.secret_key} \
   767397811928.dkr.ecr.eu-north-1.amazonaws.com/deployment-backend:latest
    EOF

  tags = {
    Name = "Django_EC2_backend_Server"
  }
}
...
resource "aws_iam_instance_profile" "ec2_profile" {
  name = "django_ec2_backend_profile"
  role = aws_iam_role.ec2_role.name
}